...
Bitmap::Bitmap(int width, int height) : _width(width), _height(height), _pPixels(new uint8_t[3*width*height]) {
}
...
bool Bitmap::write(string filename) {
    ...
    // File Header
    fileHeader.fileSize = sizeof(BitmapFileHeader) + sizeof(BitmapInfoHeader) + (3 * _width * _height);
